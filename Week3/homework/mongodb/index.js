...
async function findEpisodesExercises(client) {
  ...
  const result3a = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .find({ elements: "CLIFF" });

  const result3b = await result3a.toArray();
  const result3 = result3b.map((result) => result.title);

  console.log(`The episodes that Bob Ross painted a CLIFF are ${result3}`);

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]
  const result4a = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .find({ elements: { $all: ["CLIFF", "LIGHTHOUSE"] } });

  const result4b = await result4a.toArray();
  const result4 = result4b.map((result) => result.title);
...
async function deleteEpisodeExercise(client) {
  ...
  console.log(
    `Ran a command to delete episode and it deleted ${result6.deletedCount} episodes`
  );
}
